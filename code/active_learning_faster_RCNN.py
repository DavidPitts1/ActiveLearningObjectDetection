...
# disable autotune
ADDED_CONFIDENCE_PER_CYCLE = 1 / 11
INITAL_THRESHOLD = 0.3
AMOUNT_BOXES_INDEX = 0
NUM_CLASSES = 20
MINIMUM_BOX_THRESHOLD = 4
BOX_AREA_THRESHOLD = 15000
UNLABELED_ORACLE = 1750
NUM_TRAIN = 16551
STARTING_SIZE = 2000
ADDENDUM =  500
box_size=False
...
HARDNESS_WEIGHTS = np.array([0.04475061, 0.0420423, 0.04654893, 0.060229, 0.05693375,
                             0.04239566, 0.04038217, 0.04224999, 0.06372515, 0.04781136,
                             0.0534471, 0.0451718, 0.04319551, 0.04189276, 0.0423225,
                             0.09763746, 0.04407908, 0.05338911, 0.04235118, 0.04944457])
import logging
import time
import numpy as np
import mxnet as mx
...
def validate(net, val_data, ctx, eval_metric, args, AL=False):
...
class SubsetSequentialSampler(gluon.data.Sampler):
...
def init_datasets(args, batch_size, ctx, net):
    train_dataset, val_dataset, eval_metric = get_dataset(args.dataset, args)
    val_dataset = val_dataset.sample(SubsetSequentialSampler(np.arange(500)))
    train_data, val_data = get_dataloader(
        net, train_dataset, val_dataset, FasterRCNNDefaultTrainTransform,
        FasterRCNNDefaultValTransform, batch_size, len(ctx), args)
    return eval_metric, train_dataset, val_data


def init_active_learning_indices():
    indices = list(range(NUM_TRAIN))
    np.random.shuffle(indices)
    labeled_set = indices[:STARTING_SIZE]
    unlabeled_set = indices[STARTING_SIZE:]
...
def init_logger():
    ...
    logger = logging.getLogger()
    logger.setLevel(logging.INFO)
    log_file_path = curr_time + '_train.log'
    log_dir = os.path.dirname(log_file_path)
    if log_dir and not os.path.exists(log_dir):
        os.makedirs(log_dir)
    fh = logging.FileHandler(log_file_path)
...
def init_net(args, LOAD_NET=False, path_load=""):
    if args.amp:
        amp.init()
    # training contexts
    if args.horovod:
    ...
    if LOAD_NET:
        net.load_params(path_load, ignore_extra=True)
    else:
    ...
    return batch_size, ctx, net


def init_net_and_variables(args, net):
    ids_list = []
    scores_list = []
    indxs_list = []
    if not args.disable_hybridization:
        ...
        net.hybridize(static_alloc=args.static_alloc)
    return ids_list, indxs_list, scores_list


def validation_and_logging(args, c, ctx, eval_metric, logger, net, val_data):
    map_name, mean_ap = validate(net, val_data, ctx, eval_metric, args)
    val_msg = '\n'.join(['{}={}'.format(k, v) for k, v in zip(map_name, mean_ap)])
    logger.info('[Cycle {}] Validation: \n{}'.format(c, val_msg))
    logger.info("The current amount of pictures added is {}".format(c * 500 + 2000))


def update_indicies_and_log(labeled_set, logger, new_indexes, unlabeled_set):
    labeled_set, unlabeled_set = update_indexes(labeled_set, new_indexes, unlabeled_set)
    log_indexes_update(labeled_set, logger, new_indexes, unlabeled_set)
    return labeled_set, unlabeled_set


def log_indexes_update(labeled_set, logger, new_indexes, unlabeled_set):
    logger.info("New indexes added in the current phase :")
    logger.info(new_indexes)
    logger.info(
        "the len of the labeled set is {} and the unlabled is {}".format(len(labeled_set), len(unlabeled_set)))


def update_indexes(labeled_set, new_indexes, unlabeled_set):
    unlabeled_set = set(unlabeled_set)
    labeled_set = set(labeled_set)
    sampled_set = set(new_indexes)
    unlabeled_set.difference_update(sampled_set)
    labeled_set.update(sampled_set)
    unlabeled_set = list(unlabeled_set)
    labeled_set = list(labeled_set)
    return labeled_set, unlabeled_set


def sampling_next_cycle_data(classes_sampling, ids_list, indxs_list):
    new_indexes = []
    for i in tqdm(classes_sampling):
        j = 0
        for indx, ids_curr in zip(indxs_list, ids_list):
            if i in ids_curr:
                new_indexes.append(indx)
                indxs_list.remove(indx)
                del ids_list[j]
                break
            j += 1
    return new_indexes
...
def active_learning_cycles(args, batch_size, ctx, eval_metric, labeled_set, logger, net, train_dataset, unlabeled_set,
                           val_data, random_oracle=False):
    for cycle_num in range(10):
        oracle_data, to_eval, train_data = current_cycle_init_data(args, batch_size, ctx, labeled_set, net,
                                                                   train_dataset, unlabeled_set,cycle_num=cycle_num)

        args.epochs=5*cycle_num+5
        args.start_epoch =5*cycle_num

        train(net, train_data, oracle_data, eval_metric, batch_size, ctx, args)
        labeled_set, unlabeled_set= init_next_cycle_data(ADDENDUM, args, ctx, labeled_set, logger, net, oracle_data,
                                                          to_eval, unlabeled_set, cycle_num, random_oracle=random_oracle)

        validation_and_logging(args, cycle_num, ctx, eval_metric, logger, net, val_data)
def init_next_cycle_data(ADDENDUM, args, ctx, labeled_set, logger, net, oracle_data, to_eval, unlabeled_set, c,
                         random_oracle=False):

    ids_list, indxs_list ,bboxes_list= predict_classes_on_unlabeled(args, ctx, net, oracle_data, to_eval,c)

    if random_oracle and box_size==False and num_boxes==False:
        new_indexes = sample_next_cycle_random(ADDENDUM, unlabeled_set)
    elif box_size:
        new_indexes = sample_next_cycle_area_boxes(ADDENDUM, bboxes_list, indxs_list)
    elif num_boxes:
        new_indexes = sample_next_cycle_num_boxes(ADDENDUM, bboxes_list, c, indxs_list)
    else:
        new_indexes = sample_next_cycle_weighted(ADDENDUM, ids_list, indxs_list)

    labeled_set, unlabeled_set = update_indicies_and_log(labeled_set, logger, new_indexes, unlabeled_set)
    return labeled_set, unlabeled_set


def sample_next_cycle_weighted(ADDENDUM, ids_list, indxs_list):
    print("Weighted Sampling")
    classes_sampling = np.random.choice(np.arange(NUM_CLASSES), ADDENDUM, p=HARDNESS_WEIGHTS)
    new_indexes = sampling_next_cycle_data(classes_sampling, ids_list, indxs_list)
    return new_indexes


def sample_next_cycle_random(ADDENDUM, unlabeled_set):
    print("Random sampling")
    np.random.shuffle(unlabeled_set)
    new_indexes = unlabeled_set[:ADDENDUM]
    return new_indexes


def sample_next_cycle_area_boxes(ADDENDUM, bboxes_list, indxs_list):
    print("Size boxes")
    new_indexes = []
    i = 0
    for boxes, indx in zip(bboxes_list, indxs_list):
        ...
        if len(boxes) != 4:
            ...
            a = a.asnumpy()
            b = b.asnumpy()
            if np.abs(a * b).mean() > BOX_AREA_THRESHOLD and i < ADDENDUM:
                i += 1
                new_indexes.append(indx)
            if i > ADDENDUM:
                break
...
def sample_next_cycle_num_boxes(ADDENDUM, bboxes_list, cycle_num, indxs_list):
    new_indexes = []
    print("Num Boxes")
    i = 0
    for boxes, indx in zip(bboxes_list, indxs_list):
        boxes = boxes.asnumpy()
        if boxes.shape[AMOUNT_BOXES_INDEX] > MINIMUM_BOX_THRESHOLD + cycle_num and i < ADDENDUM:
            new_indexes.append(indx)
            i += 1
        if i > ADDENDUM:
            break
...
def predict_classes_on_unlabeled(args, ctx, net, oracle_data, to_eval, cycle_num):
    ids_list, indxs_list, scores_list = init_net_and_variables(args, net)
    bboxes_list=[]
    for batch, ind in tqdm(zip(oracle_data, to_eval)):
        batch = split_and_load(batch, ctx_list=ctx)
        for x, y, im_scale in zip(*batch):
            box_ids, scores, bboxes = net(x)
            box_ids = box_ids.asnumpy()
            scores = scores.asnumpy()
            score_indexes = np.argwhere(scores[0, :, 0] > INITAL_THRESHOLD + cycle_num * ADDED_CONFIDENCE_PER_CYCLE
                                        )[:, 0]
            ## Here I need to add an option to increase the size
            if len(score_indexes) == 0:
                continue
            ids_list.append(box_ids[0, score_indexes, 0])
            scores_list.append(scores[0, score_indexes, 0])
            bboxes_list.append(bboxes[0,score_indexes,:])
            indxs_list.append(ind)

    return ids_list, indxs_list,bboxes_list


def current_cycle_init_data(args, batch_size, ctx, labeled_set, net, train_dataset, unlabeled_set,cycle_num):
    train_dataset_demo = train_dataset.sample(SubsetSequentialSampler(labeled_set))
    np.random.shuffle(unlabeled_set)
    to_eval = unlabeled_set[:UNLABELED_ORACLE]
    orcale_dataset = train_dataset.sample(SubsetSequentialSampler(to_eval))


    train_data, oracle_data = get_dataloader(
        net, train_dataset_demo, orcale_dataset, FasterRCNNDefaultTrainTransform,
        FasterRCNNDefaultValTransform, batch_size, len(ctx), args)
    return oracle_data, to_eval, train_data
...
def main():
    ...
    batch_size, ctx, net = init_net(args, LOAD_NET=False, path_load="")
    labeled_set, unlabeled_set = init_active_learning_indices()
    eval_metric, train_dataset, val_data = init_datasets(args, batch_size, ctx, net)
    active_learning_cycles(args, batch_size, ctx, eval_metric, labeled_set, logger, net, train_dataset, unlabeled_set,
                           val_data, random_oracle=random_oracle)
